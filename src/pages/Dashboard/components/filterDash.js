import { useFilterDash } from "../../../context"

export const FilterDash = ({ setShow }) => {

    const { state, dispatch } = useFilterDash();
    return (
            ...
            <div id="drawer-disable-body-scrolling" className={`fixed z-40 h-screen p-5 overflow-y-auto bg-white w-72 dark:bg-gray-800 transition-transhtmlForm left-0 top-0 transhtmlForm-none`} tabIndex="-1" aria-labelledby="drawer-disable-body-scrolling-label" aria-modal="true" role="dialog">
                ...
                <div className="py-4 overflow-y-auto">
                    <ul className="text-slate-700 dark:text-slate-100">
                        ...
                        <li className="mt-1 mb-5">
                            <p className="font-semibold my-1">Sort by user ID</p>
                            <div className="flex items-center my-1">
                                <input onChange={() => dispatch({ type: "USER_SORT_BY", payload: { sortBy: "Ilowtohigh" } })} checked={state.sortBy === "Ilowtohigh" || false} id="id-sort-1" type="radio" value="" name="sort" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 dark:bg-gray-700 dark:border-gray-600" />
                                <label htmlFor="id-sort-1" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">ID - Low to High</label>
                            </div>
                            <div className="flex items-center my-1">
                                <input onChange={() => dispatch({ type: "USER_SORT_BY", payload: { sortBy: "Ihightolow" } })} checked={state.sortBy === "Ihightolow" || false} id="id-sort-2" type="radio" value="" name="sort" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 dark:bg-gray-700 dark:border-gray-600" />
                                <label htmlFor="id-sort-2" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">ID - High to Low</label>
                            </div>
                        </li>
                        <li className="mt-1 mb-5">
